...
//import 'package:face_diseases_app/Pages/Calender.dart';
import 'package:face_diseases_app/Pages/dashboard.dart';
import 'package:face_diseases_app/calender.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  void onSearch(String search) {
    setState(() {
      _filteredUsers = _users.where((user2) =>
        user2.username.toLowerCase().contains(search.toLowerCase()) ||
        user2.name.toLowerCase().contains(search.toLowerCase())).toList();
    });
  }
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) => userComponent(user2: _filteredUsers[index]),
        ),
      ),
    );
  }

  Widget userComponent({required User user2}) {
    return Container(
        ...
        children: [
          Row(children: [
            Container(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(50),
                child: Image.network(user2.image, fit: BoxFit.cover),
              ),
            ),
            ...
            Column(
              ...
              children: [
                Text(user2.name, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w500)),
                const SizedBox(height: 5),
                Text(user2.username, style: const TextStyle(color: Color.fromARGB(255, 81, 80, 80))),
              ],
            ),
          ]),
          GestureDetector(
            onTap: () => showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                content: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Hospital: ${user2.username}"),
                    Text("Information: ${user2.info}"),
                  ],
                ),
                actions: <Widget>[
                  TextButton(
...
class FirestoreService {
  ...
  Stream<List<User>> getUsers() {
    return firestore.collection('users2').snapshots().map((snapshot) {
