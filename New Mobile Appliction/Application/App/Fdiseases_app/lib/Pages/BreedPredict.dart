...
void main() async {
  ...
  runApp(const BreedScanPage());
}

class BreedScanPage extends StatelessWidget {
  const BreedScanPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      title: 'Scan Your Breed',
      theme: ThemeData(
...
class _TfliteModelState extends State<TfliteModel> {
  ...
  Map<String, String> diseaseDescriptions = {
    ...
    "atopic dermatitis":
    "1. Diagnosis and Assessment:Visit a healthcare provider for an accurate diagnosis, ruling out other conditions. Identify any specific triggers, such as allergens, irritants, or stress, that may worsen symptoms.\n\n"
    ...
    "Seborrheic keratoses are common, benign skin growths that often appear in older adults. Though generally harmless, they can be removed if they cause discomfort or for cosmetic reasons. Here’s a step-by-step guide for managing seborrheic keratoses:\n\n"
    ...
    "tinea ringworm":
    "A fungal infection that causes a red, itchy, scaly circular rash on the body.",
  ...
  Future loadModel() async {
    ...
    String? res = await Tflite.loadModel(
      model: "assets/Breedmodel.tflite",
      labels: "assets/Breedlabels.txt",
    );
  ...
  Future imageClassification(File image) async {
    ...
    recognitions?.forEach((result) {
      ...
      String description =
          diseaseDescriptions[normalizedLabel] ?? "No description available";
      print(
          "Label: $rawLabel, Normalized: $normalizedLabel, Description: $description");
    });
  ...
  Future<void> saveResultsToFirebase(List<dynamic>? recognitions) async {
    if (recognitions != null && recognitions.isNotEmpty) {
      final docUser =
      FirebaseFirestore.instance.collection('disease_reports').doc();
      await docUser.set({
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:
        const Text('Scan Your Breed', style: TextStyle(color: Colors.white)),
          ...
          SingleChildScrollView(
            child: Column(
              children: imageSelect && _results != null
                ...
                String description = diseaseDescriptions[label] ??
